...
const NCB_DenhaagMetaToggle = ( id = 'denhaag-share-button' ) => {
	...
	button.addEventListener( 'click', () => button.setAttribute( 'aria-expanded', 'false' === button.getAttribute( 'aria-expanded' ) ) );
	button.addEventListener( 'keydown', ( e ) => NCB_DenhaagMetaToggleWrapper( button, e ) );

	const wrapper = document.getElementById( button.getAttribute( 'aria-controls' ) );
	if ( !! wrapper ) {
		wrapper.addEventListener( 'keydown', ( e ) => NCB_DenhaagMetaToggleWrapper( button, e ) );
	}
}
