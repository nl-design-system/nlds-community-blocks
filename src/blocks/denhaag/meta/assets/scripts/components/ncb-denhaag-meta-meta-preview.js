...
const NCB_DenhaagMetaMetaPreview = ({showMeta = false}) => {
  ...
  const newPost = wp.data.select( 'core/editor' ).isCleanNewPost() || wp.data.select( 'core/editor' ).isEditedPostNew();
  ...
  return useMemo( () => {
    ...
    const cleanedData = Object.entries( metadata ).reduce( (a, [k, v]) => (v ? (a[k] = v, a) : a), {} );
    ...
    const metaItems = Object.entries( cleanedData ).map( (object) => {
      ...
      if ('object' === typeof value) {
        if (!!newPost) {
          ...
          const regex = new RegExp( "<svg[^>]*></?use[^>]*><\/svg>", "mi" );
    ...
    return <div className="denhaag-meta__meta"
                dangerouslySetInnerHTML={
                  {
                    __html: metaItems.join( '<hr class="denhaag-divider denhaag-divider--vertical" role="presentation" />' )
                  }
                } />
