...
export default function edit( { clientId, attributes, setAttributes } ) {
	...
	useLayoutEffect( () => {
		...
		if( 0 === innerBlocksLength ) {
			...
			wp.data.dispatch( 'core/block-editor' ).insertBlocks( wp.blocks.createBlock( 'ncb-denhaag/button', {} ), 0, clientId );
		}
	}, [ innerBlocksLength ] );

	const orientation = window.matchMedia( '(min-width: 480px) AND (orientation: landscape)' ).matches ? 'horizontal' : 'vertical';
