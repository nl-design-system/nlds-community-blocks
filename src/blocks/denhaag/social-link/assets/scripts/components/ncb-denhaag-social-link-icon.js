...
const NCB_DenhaagSocialLinkIcon = ( { url } ) => {
	...
	return useMemo( () => {
		if ( ! url ) {
			return _x( 'No link provided', 'denhaag/social-link icon: Notice', 'nlds-community-blocks' );
		}
		...
		return _x( 'This is an unsupported domain.', 'denhaag/social-link icon: Notice', 'nlds-community-blocks' );
	}, [ url ] );
};
