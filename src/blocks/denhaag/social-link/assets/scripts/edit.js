			...
			<span className={ _CLASSES.link }>
				{ ( ! attributes.hasOwnProperty( 'link' ) || ! attributes.link ) && (
					<>
						...
						{ _x( 'Click the icon to add a link.', 'ncb-denhaag/social-link: Instruction', 'nlds-community-blocks' ) }
						</span>
					</>
				) }
				{ attributes.link?.url && ( <NCB_DenhaagSocialLinkIcon url={ attributes.link.url } /> ) }
			</span>
		</>
	);
}
