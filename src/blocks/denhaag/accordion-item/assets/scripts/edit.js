...
const {allowed_blocks: ALLOWED_BLOCKS} = window.ncb_editor_variables?.denhaag['accordion-items'];
...
export default function Edit(
   ...
   */
  const isParentOfSelectedBlock = useSelect( (select) => select( 'core/block-editor' ).hasSelectedInnerBlock( clientId, true ), [clientId] );

  const _CLASSES = useMemo( () => {
    return {
      root: classNames( 'denhaag-accordion__container', {
        ['denhaag-accordion__container--open']: !!isSelected || !!isParentOfSelectedBlock
      } ),
   ...
   * @constructor
   */
  const Heading = `h${context['ncb-denhaag/accordion-heading'] ?? attributes.heading}`;
      ...
      <Heading className={_CLASSES.panel}>
        ...
        >
          <RichText
            ...
            onChange={(title) => setAttributes( {title: title.replace( /<[^>]*>?/gm, '' ).trim()} )}
          />
        </button>
