...
export default function edit({attributes, setAttributes, clientId}) {
  ...
  const _CLASSES = useMemo( () => {
    return {
      root: classnames( 'editor-denhaag-columns', {
        [`has-${attributes.columns}-columns`]: !!attributes.columns && !!attributes.layout,
        [`editor-denhaag-columns--${attributes.layout}`]: !!attributes.layout,
  ...
  const _TEMPLATE = useMemo( () => Array.from(
  ...
  useLayoutEffect( () => {
    if (innerBlocksLength < attributes.columns) {
      for (let i = 0, j = Math.abs( attributes.columns - innerBlocksLength ); i < j; i++) {
        wp.data.dispatch( 'core/block-editor' ).insertBlocks( wp.blocks.createBlock( 'ncb-denhaag/column', {} ), innerBlocksLength + i, clientId );
      }
    }
  }, [
    attributes.columns,
