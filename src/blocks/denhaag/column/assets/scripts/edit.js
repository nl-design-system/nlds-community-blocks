...
export default function edit({clientId, context}) {
    ...
    return blocks.filter( block => {
      if (1 === context['ncb-denhaag/columns']) {
        return (
          ...
          && (block.name.startsWith( 'core' ) || block.name.startsWith( 'ncb-' ))
          && !(block.name.startsWith( 'ncb-' ) && block.name.endsWith( '-layout' ))
        );
      }
  ...
  return (<div className={_CLASSES}><InnerBlocks allowedBlocks={_ALLOWED_BLOCKS} templateLock={false} /></div>);
}
